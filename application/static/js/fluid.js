...
function Wave() {
    ...
    /** Bubble settings */
    var MAX_BUBBLES = 0; // The maximum number of bubbles visible before FIFO is applied
    ...
    this.Initialize = function (canvasID) {
        ...
        if (canvas && canvas.getContext) {
            ...
            //timeUpdateInterval = setInterval(TimeUpdate, );
            tweetUpdateInterval = setInterval(CreateBubble, TWEETS_FREQUENCY);
            twitchInterval = setInterval(Twitch, 1000);
            TimeUpdate('')
            //DownloadTweets();
            CreateBubble();
            ResizeCanvas();
    ...
    function TimeUpdate(e) {
        ...
        gradientFill.addColorStop(0, 'rgba(0,100,250,1)');
        gradientFill.addColorStop(1, 'rgba(0,250,250,1)');
            ...
            if (b.dissolved == false) {
                ...
                while (b.children.length < b.dissolveSize) {
                    b.children.push({
                        ...
                        velocity: {x: (Math.random() * 20) - 10, y: -(Math.random() * 10)}
                    });
                }
     ...
     *
     */
    function ResizeCanvas(e, water_level) {
        ...
        for (var i = 0; i < DETAIL + 1; i++) {
            ...
            particles[i].y = HEIGHT*water_level;
